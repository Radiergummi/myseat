...
$(document).ready(function() {
		...
		$.arte({'ajax_url': '/ajax/realtime.php?lastid=<?php echo $_SESSION['max_id']; ?>', 'on_success': update_field, 'time': 5000}).start();
			function update_field(data)
			{
				if ( data > 0 ) {
				$("#realtimeupdate").html(
					"<div class='alert_warning'><p style='margin-bottom:10px;'><a href='/verwaltung/reservierungen/<?php echo $_SESSION['selectedDate']; ?>'>(" + data + ") <?php echo _new_entry;?></a></p></div>"
					);	
				};
			}
		...
		$("#datepicker").datepicker({
			...
			dateFormat: '<?php echo Config::get('general.datepickerformat');?>',
			regional: '<?php echo substr($_SESSION['language'],0,2);?>',
			onSelect: function(dateText, inst) { window.location.href="/verwaltung/reservierungen/"+$("#dbdate").val(); }
		});
