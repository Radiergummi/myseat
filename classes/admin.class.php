...
class flexibleAccess
{
  ...
  * var string
  */
 public $sessionVariable = 'XTSess';
 /*
  * The database table that holds all the information
  * var string
  */
 public $dbTable = 'plc_users';
 /*
  * The database table that holds all sessions
  * var string
  */
 public $dbSession = 'plc_autologin';
 /*
  * The database table that holds all sessions
  * var string
  */
 public $dbAttempts = 'plc_sessions';
 /*
...
public $tbLogin = array(
  ...
  * var string
  */
public $dbTrans = 'plc_trans';
    public $tbTrans = array(
        'id' => 'id',
        'field' => 'type',
        'app' => 'app',
        'old_id' => 'old_id',
...
// var $remTime = 10800; // 3 hours
  ...
  */
 public $majorsalt = 'N;8m0(PSOZqzlg9?cLt5]:ZT7|0Fo6w$A*pAQ`O8<.^vZr6@!EY~lI{lX|Ty)-+';
 /*
...
*/
  ...
  * @param string $dbConn
  */
 public function flexibleAccess($dbConn = '')
 {
    $this->dbHost					= Config::get('db.host');
    $this->dbPort					= Config::get('db.port');
    $this->dbUser					= Config::get('db.user');
    $this->dbPass					= Config::get('db.password');
    $this->dbName					= Config::get('db.name');
    $this->dbTablePrefix	= Config::get('db.prefix');
 ...
 * @return bool
 */
public function login($uname, $password, $newpassword)
{
 ...
 * @return int
 */
public function newpassword($newpass = '', $id)
{
    if (!empty($newpass)) {
        $password = $newpass;
        switch (strtolower($this->passMethod)) {
        ...
        $id = (int) mysqli_insert_id($this->dbConn);
    }
...
public function insertSession($data)
{
 ...
 public function logout($redirectTo = '')
 {
     if (isset($_COOKIE[$this->remCookieName])) {
         ...
         foreach ($setCookie as $k => $v) {
             $setCookie[$k] = "'".$this->escape($v)."'";
         }
  ...
  * return string
  */
public function ramdomPassword($pw_length = 6, $use_caps = false, $use_numeric = true, $use_specials = false)
{
    $caps = array();
    $numbers = array();
    $num_specials = 0;
    ...
    if ($use_specials) {
        $signs = array(33,35,36,37,38,42,43,45,46);
    } // create signs
 ...
 * @return bool
 */
public function uniquePassword($password)
{
...
public function pwEncode($password)
{
    /*
    * Password salt
 ...
 * @author Bernd
 **/
public function hash_password_db($password_db = false, $password = false)
{
    $salt_length = $this->salt_length;
 ...
 * @param array $data
 */
public function autologin_weak()
{
    if (isset($_COOKIE[$this->remCookieName])) {
        $setCookie = unserialize(base64_decode($_COOKIE[$this->remCookieName]));
    //debugging
...
public function autologin()
{
    if (isset($_COOKIE[$this->remCookieName])) {
     ...
     foreach ($setCookie as $k => $v) {
        ...
        $this->sessionData = mysqli_fetch_array($res);
...
public function read_cookie()
{
  ...
  * return int
  */
public function failedAttempt($id = 1)
{
 ...
 * return int
 */
public function clearAttempt($id = 1)
{
  ...
  * return bool
  */
public function checkAttempt($id = 1)
{
 ...
 * @return int
 */
public function transformData($app = 'Unknown', $field, $id)
{
 ...
 * @return bool
 */
public function buildPersons($needle)
{
    $haystack = array();
 ...
 */
public function writePersons($data)
{
...
*/
   ...
   * @return string
   */
  public function query($sql, $line = 'Unknown')
  {
 ...
 * @return bool
 */
public function loadUserDetail($userID)
{
    ...
    $this->userDetail = mysqli_fetch_array($res);
 ...
 * @return bool
 */
public function loadUserDetailNew($appID, $userID)
{
    $this->userDetail = array();
  ...
  public function escape($str)
  {
      ...
      $str = (is_array($str)) ? $str : mysqli_real_escape_string($this->dbConn, $str);
   ...
   * @return string
   */
  public function roles_dropdown($roles, $selected = 4)
  {
   ...
   * @return bool
   */
  public function error($error, $line = '', $die = false)
  {
