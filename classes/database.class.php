...
function escapeInput($str) {
	...
	$str = (is_array($str)) ? $str : mysqli_real_escape_string(Config::get('db.link'), $str);
	return $str;
}
...
function secureSuperGlobals() {
	...
	foreach ($_POST as $key => $value) {
		if ( $key != 'description' ) {
			...
			$value = (is_array($value)) ? $value : mysqli_real_escape_string(Config::get('db.link'), $value);
			$_POST[$key] = $value;
		}
	}
...
function query($query) {
	...
	$result = mysqli_query(Config::get('db.link'), $query) or die ("Error in query: $query. ".mysqli_error(Config::get('db.link')));
	return $result;
}
...
function writeForm($table ='') {
	...
	if (!empty($table)) {
		...
		//echo $query;
		$result = query($query);
		$new_id = mysqli_insert_id(Config::get('db.link'));
